...
//! * The denoiser and the saturation grade also run on the unfinished image, so
//!   an interactive viewport is filtered while the camera is moving rather than
//!   only once it stops -- see [`renderer::RenderConfig::preview`]
//!
//! ## Example:
//! ```rust
