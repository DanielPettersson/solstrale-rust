...
/// A smooth glass ball on a plain grey floor under one small bright light: a
/// caustic, and as little else as the scene can be made of.
///
/// Deliberately not `create_specular_scene`. That floor is textured on purpose,
/// and a masked error over it would measure the texture as much as the
/// caustic.
///
/// The ball rests on the floor rather than floating. A sphere of radius `r` and
/// index 1.5 focuses a parallel beam about `1.5 r` from its centre, which is
/// half a radius past its own surface, so the bright spot a glass ball throws
/// is always tight against the contact point -- there is no placement that
/// separates them, and the camera is low so that the near edge of the spot is
/// not hidden by the ball itself.
///
/// The light's radiance is 8 for the reason `create_rough_glass_scene` records
/// at length: bright enough and the firefly clamp becomes the thing being
/// measured. A caustic path carries at most `floor albedo * radiance` here,
/// which is half the threshold.
#[allow(dead_code)]
pub fn create_caustic_scene(render_config: RenderConfig) -> Scene {
    let nop = NopTransformer();
    let world: Vec<Hittables> = vec![
        Quad::new(
            Vec3::new(-8., 0., -8.),
    ...
    Scene {
        ...
        camera: CameraConfig {
            vertical_fov_degrees: 30.,
            aperture_size: 0.,
            look_from: Vec3::new(0., 1.6, 6.),
            look_at: Vec3::new(0., 0.4, 0.),
            up: Vec3::new(0., 1., 0.),
        },
        // Black: the only thing lighting this scene is the quad, so the floor
        // shows direct light and the caustic and nothing else.
        background_color: ZERO_VECTOR,
        render_config,
    }
}

/// Geometry of [`create_caustic_scene`], so a test can mask the ball and the
/// patch of floor its caustic lands on without re-deriving where they are.
#[allow(dead_code)]
pub const CAUSTIC_SPHERE_RADIUS: f64 = 1.;
#[allow(dead_code)]
pub const CAUSTIC_SPHERE_CENTER: Vec3 = Vec3 {
    x: 0.,
    y: CAUSTIC_SPHERE_RADIUS,
    z: 0.,
};
